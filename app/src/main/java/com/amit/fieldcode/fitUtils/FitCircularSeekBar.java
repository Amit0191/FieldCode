...
public class FitCircularSeekBar extends View {
    /**
    ...
    public FitCircularSeekBar(Context context) {
        super(context);
        init(null, 0);
    }

    public FitCircularSeekBar(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(attrs, 0);
    }

    public FitCircularSeekBar(Context context, AttributeSet attrs, int defStyle) {
    ...
    private void initAttributes(TypedArray attrArray) {
        mCircleXRadius = attrArray.getDimension(R.styleable.fit_cs_CircularSeekBar_cs_circle_x_radius, DEFAULT_CIRCLE_X_RADIUS);
        mCircleYRadius = attrArray.getDimension(R.styleable.fit_cs_CircularSeekBar_cs_circle_y_radius, DEFAULT_CIRCLE_Y_RADIUS);
        mPointerStrokeWidth = attrArray.getDimension(R.styleable.fit_cs_CircularSeekBar_cs_pointer_stroke_width, DEFAULT_POINTER_STROKE_WIDTH);
        mPointerHaloWidth = attrArray.getDimension(R.styleable.fit_cs_CircularSeekBar_cs_pointer_halo_width, DEFAULT_POINTER_HALO_WIDTH);
        mPointerHaloBorderWidth = attrArray.getDimension(R.styleable.fit_cs_CircularSeekBar_cs_pointer_halo_border_width, DEFAULT_POINTER_HALO_BORDER_WIDTH);
        mCircleStrokeWidth = attrArray.getDimension(R.styleable.fit_cs_CircularSeekBar_cs_circle_stroke_width, DEFAULT_CIRCLE_STROKE_WIDTH);

        int circleStyle = attrArray.getInt(R.styleable.fit_cs_CircularSeekBar_fit_cs_circle_style, DEFAULT_CIRCLE_STYLE);
        mCircleStyle = Paint.Cap.values()[circleStyle];

        mPointerColor = attrArray.getColor(R.styleable.fit_cs_CircularSeekBar_cs_pointer_color, DEFAULT_POINTER_COLOR);
        mPointerHaloColor = attrArray.getColor(R.styleable.fit_cs_CircularSeekBar_cs_pointer_halo_color, DEFAULT_POINTER_HALO_COLOR);
        mPointerHaloColorOnTouch = attrArray.getColor(R.styleable.fit_cs_CircularSeekBar_cs_pointer_halo_color_ontouch, DEFAULT_POINTER_HALO_COLOR_ONTOUCH);
        mCircleColor = attrArray.getColor(R.styleable.fit_cs_CircularSeekBar_cs_circle_color, DEFAULT_CIRCLE_COLOR);
        mCircleProgressColor = attrArray.getColor(R.styleable.fit_cs_CircularSeekBar_cs_circle_progress_color, DEFAULT_CIRCLE_PROGRESS_COLOR);
        mCircleFillColor = attrArray.getColor(R.styleable.fit_cs_CircularSeekBar_cs_circle_fill, DEFAULT_CIRCLE_FILL_COLOR);

        mPointerAlpha = Color.alpha(mPointerHaloColor);

        mPointerAlphaOnTouch = attrArray.getInt(R.styleable.fit_cs_CircularSeekBar_cs_pointer_alpha_ontouch, DEFAULT_POINTER_ALPHA_ONTOUCH);
        if (mPointerAlphaOnTouch > 255 || mPointerAlphaOnTouch < 0) {
            mPointerAlphaOnTouch = DEFAULT_POINTER_ALPHA_ONTOUCH;
        }

        mMax = attrArray.getInt(R.styleable.fit_cs_CircularSeekBar_cs_max, DEFAULT_MAX);
        mProgress = attrArray.getInt(R.styleable.fit_cs_CircularSeekBar_cs_progress, DEFAULT_PROGRESS);
        mCustomRadii = attrArray.getBoolean(R.styleable.fit_cs_CircularSeekBar_cs_use_custom_radii, DEFAULT_USE_CUSTOM_RADII);
        mMaintainEqualCircle = attrArray.getBoolean(R.styleable.fit_cs_CircularSeekBar_cs_maintain_equal_circle, DEFAULT_MAINTAIN_EQUAL_CIRCLE);
        mMoveOutsideCircle = attrArray.getBoolean(R.styleable.fit_cs_CircularSeekBar_cs_move_outside_circle, DEFAULT_MOVE_OUTSIDE_CIRCLE);
        mLockEnabled = attrArray.getBoolean(R.styleable.fit_cs_CircularSeekBar_cs_lock_enabled, DEFAULT_LOCK_ENABLED);
        mDisablePointer = attrArray.getBoolean(R.styleable.fit_cs_CircularSeekBar_cs_disable_pointer, DEFAULT_DISABLE_POINTER);
        mNegativeEnabled = attrArray.getBoolean(R.styleable.fit_cs_CircularSeekBar_cs_negative_enabled, DEFAULT_NEGATIVE_ENABLED);
        mIsInNegativeHalf = false;
        mDisableProgressGlow = attrArray.getBoolean(R.styleable.fit_cs_CircularSeekBar_cs_disable_progress_glow, DEFAULT_DISABLE_PROGRESS_GLOW);
        mHideProgressWhenEmpty = attrArray.getBoolean(R.styleable.fit_cs_CircularSeekBar_cs_hide_progress_when_empty, DEFAULT_CS_HIDE_PROGRESS_WHEN_EMPTY);

        // Modulo 360 right now to avoid constant conversion
        mStartAngle = ((360f + (attrArray.getFloat((R.styleable.fit_cs_CircularSeekBar_cs_start_angle), DEFAULT_START_ANGLE) % 360f)) % 360f);
        mEndAngle = ((360f + (attrArray.getFloat((R.styleable.fit_cs_CircularSeekBar_cs_end_angle), DEFAULT_END_ANGLE) % 360f)) % 360f);
...
//            }
    ...
    public interface OnCircularSeekBarChangeListener {
        ...
        public abstract void onProgressChanged(FitCircularSeekBar circularSeekBar, float progress, boolean fromUser);

        public abstract void onStopTrackingTouch(FitCircularSeekBar seekBar);

        public abstract void onStartTrackingTouch(FitCircularSeekBar seekBar);
    }
}
